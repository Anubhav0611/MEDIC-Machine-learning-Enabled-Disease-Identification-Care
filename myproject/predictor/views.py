...
model = joblib.load(r'D:\DISEASE_PREDICTION_FINAL\random_forest_model.pkl')
  # adjust this path
...
def predict(request):
    ...
    if request.method == 'POST':
        ...
        # Normalize symptoms to snake_case
        normalized_symptoms = [s.strip().lower().replace(" ", "_").replace("-", "_") for s in selected_symptoms]

        # Build input vector (1 if symptom present, else 0)
